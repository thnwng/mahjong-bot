...
| Path | What |
|---|---|
| `app/` | Next.js 15 App Router shell. Styling = the **Halcyon design system**: `app/halcyon.css` (vendored tokens from `E:\Claude\halcyon-ds`) + `globals.css` (classes driven by Halcyon tokens/radii/shadows; the FONT tokens are overridden in globals.css `:root` — the whole app is Noto Serif since 2026-07-30, halcyon.css itself untouched so a re-vendor won't clobber it). Light/dark set on `<html data-theme>` — follows Telegram's colorScheme on a real launch, else the OS (`layout.tsx` boot script + `lib/telegram.ts`); accent `data-accent="slate"`. Re-vendor by re-concatenating the halcyon-ds token files into `app/halcyon.css`. |
