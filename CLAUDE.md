...
| Path | What |
|---|---|
| `app/` | Next.js 15 App Router shell. Styling = the **Halcyon design system**: `app/halcyon.css` (vendored tokens from `E:\Claude\halcyon-ds`) + `globals.css` (classes driven by Halcyon tokens/radii/shadows; the FONT tokens are overridden in globals.css `:root` — the whole app is Noto Serif since 2026-07-30, halcyon.css itself untouched so a re-vendor won't clobber it. The same `:root` block holds the **comfort scale** (2026-07-30): the small end of the type scale is bumped one step (`--text-sm` 13->14, `--text-base` 15->16, `--text-xl` 20->22) and `--leading-normal` is 1.6, with matching padding/gap bumps in the rules below — the app read too cramped on a phone. Tune density there, not per-class). Light/dark set on `<html data-theme>` — follows Telegram's colorScheme on a real launch, else the OS (`layout.tsx` boot script + `lib/telegram.ts`); accent `data-accent="slate"`. Re-vendor by re-concatenating the halcyon-ds token files into `app/halcyon.css`. |
...
| `supabase/migrations/` | Numbered migrations (0001 = baseline, 0002 = sessions/prefs/presets, 0003 = display-name (drop username uniqueness), 0004 = link-first groups: `sessions.players` + `members.name` nullable + `rename_player` follows into sessions; 0005 = atomic `settle_debt()` RPC for debt settlement; 0006 = `trackers.tai_scores` jsonb for per-group winning-hand scoring; 0007 = `sessions.name` + `remove_player` RPC for the group-screen rebuild; 0008 = per-session settle: a 6-arg `settle_debt()` overload that scopes the net to one session and stamps the repayment's `session_id`; 0009 = pin `search_path = public, pg_temp` on all SECURITY DEFINER functions — linter hardening, behavior-preserving); apply new ones in the SQL editor BEFORE the matching function deploys |
